...
\begin{tikzpicture}[remember picture, overlay]
    ...
    \node[anchor=south, yshift=1.2cm, align=center] at (current page.south) {
        \textbf{\textcolor{primary}{\small PRÉSENTÉ PAR :}} \small Hicham Moussaid \\
        \scriptsize Master SIE --- ENS Meknès \\
        \vspace{0.2cm}
        \textbf{\textcolor{primary}{\small ENCADRÉ PAR :}} \small Pr. Abdelaaziz Hessane
    };
...
\begin{frame}{État d'Avancement : Sprints 0 à 9 Validés}
    ...
    \begin{imageframe}
        ...
        \begin{tabular}{clll}
            ...
            \rowcolor{accent!10}
            S9 & \textbf{Coach IA (LLM)} & \cmark & \textbf{Llama 3.2 3B + Prompts Directs} \\
            \midrule
            S10 & Application Web & \dots & \textit{Prochain sprint (Accès complet + Profils)} \\
            \bottomrule
        \end{tabular}
    \end{imageframe}
\end{frame}
...
\begin{frame}{Sprint 6 : Scoring Multimodal}
    \begin{columns}[T]
        ...
        \begin{column}{0.48\textwidth}
            \begin{imageframe}
                \footnotesize
                \textbf{Formule du Score Global (Pondérée) :}\\[0.2cm]
                \scriptsize
                $\text{Score} = (0.3 \times \text{Voix} + 0.35 \times \text{Regard} + 0.25 \times \text{Posture} + 0.1 \times \text{Scène}) \times 10$
            \end{imageframe}
            \vspace{0.3cm}
...
\begin{frame}{Sprint 9 : Migration du LLM (1B $\rightarrow$ 3B)}
    ...
    \begin{tcolorbox}[colback=secondary, colframe=primary!30]
        \centering \small
        \textbf{Stratégie ``Prompts Directs''} : Au lieu d'une conversation, on impose au modèle un \textbf{formatage strict} sans politesse inutile. Trois appels séquentiels : \textbf{Bilan} $\rightarrow$ \textbf{Conseil} $\rightarrow$ \textbf{Motivation}.
    \end{tcolorbox}
\end{frame}
...
\begin{frame}{Optimisation du Temps de Traitement (RTF)}
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.33\textwidth}
            \begin{tcolorbox}[title=\textbf{Levier Audio}, colframe=primary!60, colback=primary!3]
                \scriptsize
                \begin{itemize}
                    \item \textbf{Whisper Medium $\rightarrow$ Small} : Réduction drastique du temps d'éocution sans perte majeure de précision.
                    \item \textbf{Beam size} $5 \rightarrow 1$ : teste 1 seule hypothèse au lieu de 5 par mot.
                    \item \textbf{INT8} (dès S1) : réduit la précision des calculs (32 bits $\rightarrow$ 8 bits).
                \end{itemize}
            \end{tcolorbox}
        \end{column}
        \begin{column}{0.33\textwidth}
            \begin{tcolorbox}[title=\textbf{Levier Vision}, colframe=accent!60, colback=accent!3]
                \scriptsize
                \begin{itemize}
                    \item \textbf{Downscaling} $720 \text{p} \rightarrow 360\text{p}$ : réduit la taille des images avant analyse.
                    \item \textbf{1 fps} (dès S0) : extrait 1 image/sec au lieu de 30 (suffisant pour regard/posture).
                \end{itemize}
            \end{tcolorbox}
        \end{column}
        \begin{column}{0.33\textwidth}
            \begin{tcolorbox}[title=\textbf{Levier Pipeline}, colframe=success!60, colback=success!3]
                \scriptsize
                \begin{itemize}
                    \item \textbf{ThreadPoolExecutor} : Audio et Vision s'exécutent en parallèle au lieu de séquentiellement.
                \end{itemize}
            \end{tcolorbox}
        \end{column}
    \end{columns}
    \vspace{0.2cm}
    \begin{itemize} \scriptsize
        \item \textbf{Cold Start ($\sim$45s) :} Actuellement, les modèles sont rechargés à chaque exécution. \textbf{Sprint 10 (FastAPI)} résoudra ce problème : les modèles resteront en mémoire sur le serveur.
        \item \textbf{Prévu Sprint 10 :} Possibilité de forcer la langue dans l'interface (évite $\sim$30s de détection auto par Whisper).
    \end{itemize}
    \vspace{0.2cm}
    \begin{center}
...
\begin{frame}{Sprint 8 : Gestion Intelligente de la Mémoire}
    ...
    \begin{tikzpicture}[
        ...
        \node[phase=accent, below=0.2cm of p1] (p2) {\textbf{Phase 2}\\MediaPipe + OpenCV\\$\sim$0.8 Go};
        
        % Group Phase 1 & Phase 2
        \node[draw, dashed, primary, rounded corners, fit=(p1) (p2), inner sep=0.2cm, label=above:\scriptsize\textbf{Parallèle (Pic $\sim$2.3 Go)}] (group) {};
        
        \node[phase=success, right=1.5cm of group] (p3) {\textbf{Phase 3}\\Sentence-Transformers\\$\sim$0.5 Go};
        \node[phase=warning, right=1.5cm of p3] (p4) {\textbf{Phase 4}\\Ollama (LLM)\\$\sim$2 Go};
        
        \draw[arrow] (group) -- node[above, destroy] {\ding{55} Libéré} (p3);
    ...
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{tcolorbox}[colframe=warning, title=Hallucinations du LLM 1B, colback=warning!3]
                \scriptsize
                \textbf{Problème :} Le modèle 1B confondait forces/faiblesses et ajoutait du bla-bla conversationnel.
                
                \textbf{Solution :} Migration vers 3B + stratégie de \textbf{Prompts Directs \& Formatage Strict}.
            \end{tcolorbox}
            \vspace{0.15cm}
...
\begin{frame}{Prochain Jalon : Du Code au Produit (Sprints 10-11)}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{tcolorbox}[title=\textbf{Amélioration Continue}, colframe=accent, colback=accent!3]
                \scriptsize
                En parallèle des nouveaux sprints, un travail de \textbf{refinement} permanent sera mené pour garantir un produit final d'excellence :
                \begin{itemize}
                    \item Optimisation continue du scoring.
                    \item Ajustement des prompts du coach.
                    \item Tests sur un corpus de vidéos variées.
                \end{itemize}
            \end{tcolorbox}
            \vspace{0.2cm}
            \textbf{\textcolor{primary}{Sprint 10 : Application Web Complète}}
            \begin{itemize} \footnotesize
                \item Système complet : Inscription, Authentification, Profil.
                \item Dashboard interactif : Upload et rapports dynamiques.
            \end{itemize}
            \vspace{0.2cm}
            \textbf{\textcolor{primary}{Sprint 11 : Historique}}
            \begin{itemize} \footnotesize
                \item Suivi de la progression session après session.
                \item Analyse comparative des métriques.
            \end{itemize}
        \end{column}
        \begin{column}{0.48\textwidth}
            \begin{tcolorbox}[colback=primary!5, colframe=primary]
                ...
                \footnotesize
                Un coach de prise de parole \textbf{fonctionnel et testé}, capable d'analyser une vidéo et de générer un feedback expert, le tout \textbf{100\% local}.
            \end{tcolorbox}
            \vspace{0.3cm}
            \begin{imageframe}
                \centering \scriptsize
                \textbf{Stack Technologique (En réflexion) :}\\[0.15cm]
                \begin{tabular}{ll}
