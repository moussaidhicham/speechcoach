...
\pagestyle{fancy}
\fancyhf{}
\lhead{Rapport d'Avancement n°1 - SpeechCoach}
...
\begin{itemize}
    ...
    \item \textbf{Modèles intégrés :} Intégration de \textbf{faster-whisper} (int8) \cite{radford2022robust} pour une transcription rapide et précise de la parole, et de \textbf{MediaPipe} (Face Mesh, Hands, Pose) \cite{mediapipe2023} pour le tracking facial, l'axe du regard et l'intensité gestuelle.
    \item \textbf{Extraction de métriques :}
    \begin{itemize}
        \item \textit{Audio} : Débit de parole (WPM), détection de pauses ($>0.5\text{s}$) via l'énergie RMS (\textbf{Librosa} \cite{mcfee2015librosa}), comptage de fillers.
        \item \textit{Vision} : Ratio de présence visage, vérification de luminosité/flou, estimation de l'Eye Contact (Yaw/Pitch estimé via \texttt{solvePnP} utilisant \textbf{OpenCV} \cite{opencv_library}), ratio de visibilité des mains et intensité des mouvements corporels.
    \end{itemize}
\end{itemize}
